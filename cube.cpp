...
// In cube.h: add std::string getColorCode(std::string face, int row, int col);

// In cube.cpp
std::string Cube::getColorCode(std::string face, int row, int col) {
    if (face == "F") return Front[row][col];
    if (face == "B") return Bottom[row][col]; // Your code calls Back 'Bottom'
    if (face == "U") return Up[row][col];
    if (face == "D") return Down[row][col];
    if (face == "R") return Right[row][col];
    if (face == "L") return Left[row][col];
    return "";
}